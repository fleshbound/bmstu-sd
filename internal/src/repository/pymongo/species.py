...
from repository.pymongo.model.species import SpeciesODM
from repository.utils.types import object_id_from_int, int_from_object_id
from utils.exceptions import NotFoundRepoError


class PymongoSpeciesRepository(ISpeciesRepository, PymongoBaseRepository):
    client: MongoClient
    collection: str = "species"
    ...
    def get_by_group_id(self, group_id: NonNegativeInt) -> List[SpeciesSchema]:
        res = list(self.database[self.collection].find({'groupId': group_id}))
        return [SpeciesODM(**r).to_schema() for r in res]

    def get_all(self, skip: int = 0, limit: Optional[int] = None) -> List[SpeciesSchema]:
    ...
    def get_by_id(self, id: NonNegativeInt) -> SpeciesSchema:
        ...
        return SpeciesODM(**res).to_schema()
    ...
    def create(self, other: SpeciesSchema) -> SpeciesSchema:
        ...
        new_species = self.database[self.collection].insert_one(document=model_in_json)
        created = self.database[self.collection].full_one({'_id': int_from_object_id(new_species.inserted_id)})
        return SpeciesODM(**created).to_schema()
