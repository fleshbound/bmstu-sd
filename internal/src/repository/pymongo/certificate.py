...
from repository.pymongo.model.certificate import CertificateODM
from repository.utils.types import object_id_from_int, int_from_object_id
from utils.exceptions import NotFoundRepoError


class PymongoCertificateRepository(ICertificateRepository, PymongoBaseRepository):
    client: MongoClient
    collection: str = "certificate"
    ...
    def get_all(self, skip: int = 0, limit: Optional[int] = None) -> List[CertificateSchema]:
        ...
        return [CertificateODM(**r).to_schema() for r in res]
    ...
    def get_by_id(self, id: NonNegativeInt) -> CertificateSchema:
        ...
        return CertificateODM(**res).to_schema()
    ...
    def create(self, other: CertificateSchema) -> CertificateSchema:
        ...
        new_certificate = self.database[self.collection].insert_one(document=model_in_json)
        created = self.database[self.collection].full_one({'_id': int_from_object_id(new_certificate.inserted_id)})
        return CertificateODM(**created).to_schema()
