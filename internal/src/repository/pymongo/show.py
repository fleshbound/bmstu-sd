...
class PymongoShowRepository(IShowRepository, PymongoBaseRepository):
    ...
    def get_by_breed_id(self, breed_id: NonNegativeInt) -> List[ShowSchema]:
        ...
        return [ShowODM(**r).to_schema() for r in res]
    ...
    def get_by_standard_id(self, standard_id: NonNegativeInt) -> List[ShowSchema]:
        res = list(self.database[self.collection].find({'standardId': standard_id}))
        return [ShowODM(**r).to_schema() for r in res]

    def get_all(self, skip: int = 0, limit: Optional[int] = None) -> List[ShowSchema]:
    ...
    def get_by_id(self, id: NonNegativeInt) -> ShowSchema:
        ...
        return ShowODM(**res).to_schema()

    def create(self, other: ShowSchema) -> ShowSchema:
        other_dict = self.get_dict(other, exclude=['id'])
        model_in_json = json.dumps(other_dict)
        new_show = self.database[self.collection].insert_one(document=model_in_json)
        created = self.database[self.collection].full_one({'_id': int_from_object_id(new_show.inserted_id)})
        return ShowODM(**created).to_schema()
